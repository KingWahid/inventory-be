...
**Repository conventions (backend developers):** follow [`CLAUDE.md`](CLAUDE.md), which points to [`docs/conventions/codebase-conventions.md`](docs/conventions/codebase-conventions.md); the topic index is [`docs/README.md`](docs/README.md). Verification flow and quality gates are summarized in [§17.1](#171-quality-gates-and-development-workflow). Optional agent workflow: [`.claude/CLAUDE.md`](.claude/CLAUDE.md).
...
Docs under [`docs/`](docs/) often follow the **Industrix** layout (`pkg/services/...`, **`organization_id`** in examples). This **inventory** repo simplifies as follows — principles in [`docs/conventions/codebase-conventions.md`](docs/conventions/codebase-conventions.md) (errors, logging, context, GORM) still apply; **file locations** differ.

| Topic in `docs/` | Location / behavior in this repo |
|------------------|-----------------------------------|
| **Service** layer under `pkg/services/<bounded>/` | **`services/inventory/domains/<domain>/usecase`** (plus e.g. `services/authentication/service`) |
| **`organization_id`** in sample queries / JWT | **`tenant_id`** in DB schema and JWT claims ([`pkg/common/jwt/claims.go`](pkg/common/jwt/claims.go)) — consistent with §6–§8 |
| OpenAPI **stub ↔ domain** converters in `services/*/api/converters.go` | Each service has **`api/`**, **`openapi/`**, **`stub/`** (generated); inventory adds **`domains/*/handler`** calling usecases — follow [`docs/service/how-to-structure-openapi.md`](docs/service/how-to-structure-openapi.md) and adapt paths |
| Repositories under `pkg/database/repositories/<aggregate>/` | Data access often lives in **`services/inventory/domains/*/repository`**; **`pkg/database/base`** remains shared; consolidating under `pkg/database/repositories` is an optional evolution |
| Fx module file named **`MODULE.go`** (uppercase) | Domain modules use **`module.go`** (lowercase) — cosmetic naming only; contents are still `fx.Module` / `fx.Options` |
...
### Dependency rules and doc links

- **Single responsibility per layer** — Handlers must not import `gorm.DB`; usecases must not import `echo.Context`. Matches separation of concerns in [`.claude/rules/general/principles.md`](.claude/rules/general/principles.md).
- **HTTP & API contracts** — binding, status codes, handler boundaries: [`docs/service/how-to-write-handlers.md`](docs/service/how-to-write-handlers.md) (map paths to `services/<svc>/api` and `domains/*/handler`).
- **Transactions** — commit/rollback orchestration from usecase: [`docs/service/how-to-use-transactions.md`](docs/service/how-to-use-transactions.md); context propagation per [`docs/conventions/codebase-conventions.md`](docs/conventions/codebase-conventions.md).
- **Repositories** — interface + implementation pattern: [`docs/repository/how-to-create-a-repository.md`](docs/repository/how-to-create-a-repository.md); current code lives in **`services/inventory/domains/*/repository`** and may later consolidate into `pkg/database/repositories`.
...
### 15.1 Evolving toward `CustomError`

This document illustrates domain errors with the `AppError` struct above. [`docs/conventions/codebase-conventions.md`](docs/conventions/codebase-conventions.md) recommends **`common.NewCustomError(...).WithMessageID(...).WithErrorCode(...).WithHTTPCode(...)`** for i18n and standardized codes. **Long-term goal:** converge raw handler errors on one pattern (CustomError + `errorcodes`) so two parallel systems do not linger undocumented. Existing code (`pkg/common/errorcodes`, inventory/authentication handlers) is the baseline for incremental migration.

### 15.2 Testing strategy

- Primary guide: [`docs/conventions/unit-test.md`](docs/conventions/unit-test.md) (testify/suite, sqlmock, `integration` / `integration_all` tags).
- Per-domain usecase tests: patterns from [`docs/conventions/service-layer-test.md`](docs/conventions/service-layer-test.md), with tests alongside sources under **`services/inventory/domains/*/*_test.go`**.
- Coverage targets from conventions: prioritize repository and service/usecase layers (~80% as documented).
...
Summary from [`.claude/CLAUDE.md`](.claude/CLAUDE.md) and [`.claude/rules/general/verification-pipeline.md`](.claude/rules/general/verification-pipeline.md) — useful for developers and optional for agent workflows:
...
| **Tests** | With dev Postgres up: **`make test-all`**; otherwise **`make test`** (unit only). See also [`docs/conventions/unit-test.md`](docs/conventions/unit-test.md). |
| **Migrations / seed** | If you change `infra/database/migrations/` or mock seeds, verify apply + rollback per `.claude/rules/general/verification-pipeline.md`. |
| **Agent workflow (optional)** | Domain split `db-dev` / `biz-dev` / `app-dev` / `infra-dev` under `.claude/agents/` — not required outside Claude Code. |
